...
const AdminPanel = ({ onClose }: AdminPanelProps) => {
  ...
  // 📤 Upload (escala automática) - ✅ SUPORTA .ZIP, .M3U, .M3U8
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !workerRef.current) return;

    // Validar extensão do arquivo
    const validExtensions = ['.zip', '.m3u', '.m3u8'];
    const fileExtension = file.name.toLowerCase().match(/\.\w+$/)?.[0];
    
    if (!fileExtension || !validExtensions.includes(fileExtension)) {
      toast({
        title: "Arquivo inválido",
        description: "Por favor, selecione um arquivo .zip, .m3u ou .m3u8",
        variant: "destructive",
      });
      return;
    }
  ...
  const handlePublish = () => {
    console.log("📤 Publicando conteúdo do preview...");
    publishContent();
    toast({
      title: "Publicado!",
      ...
      >
        ...
        >
          ...
          <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            {/* UPLOAD - ✅ SUPORTA .ZIP, .M3U, .M3U8 */}
            <div>
              <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
                <FileArchive className="w-4 h-4" />
                Formatos suportados: .zip, .m3u, .m3u8
              </h3>
              <input
                ref={fileInputRef}
                type="file"
